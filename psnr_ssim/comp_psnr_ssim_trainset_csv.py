...
if __name__ == "__main__":
    ...
    f_read.close()
    results_list.sort(key=lambda x:(len(x.split()[0]), x.split()[0]))
    ...
    scan = [result.split()[0] for result in results_list[1:]]
    psnr_ssim_result = np.transpose(np.vstack((noise_psnr_list_all, noise_ssim_list_all)))

    # creat DF and save .csv file
    df = pd.DataFrame(psnr_ssim_result, index=scan, columns=['PSNR', 'SSIM'])
    psnr_ssim_mean = df.mean()
    df.loc['mean'] = psnr_ssim_mean
    df.to_csv(os.path.join(args.noise_dir, "psnr_ssim.csv"))
